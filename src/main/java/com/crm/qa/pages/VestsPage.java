...
public class VestsPage extends TestBase {
    ...
    List <String> VestsProductNamelst = new ArrayList<>();
   // List<String> VestsProductPricelst = new ArrayList<>();
    List<String> VestsImageUrllst = new ArrayList<>();
    List<String> VestsAffiliateLinks = new ArrayList<>();

    String pageLink = "https://www.amazon.co.uk/Best-Sellers-Clothing-Mens-Vests/zgbs/clothing/1731029031/ref=zg_bs_nav_ap_4_1730998031";

  public VestsPage() throws FileNotFoundException {
        PageFactory.initElements(driver, this);
    }

    private void goToVestsCatalogPage()
    {
        driver.navigate().to(pageLink);
    }

    public void getVestsdata() throws FileNotFoundException, InterruptedException {

      goToVestsCatalogPage();
      waitUntilPageReady();
        ...
        for (int i=1;i<=26;i++) {
            try {
               if(i!=7){
                    ...
                    VestsImageUrllst.add(imagelnk);
                }
            } catch (Exception e) {
                e.printStackTrace();
                VestsImageUrllst.add("N/A");
            }
        }
    }
    ...
    private void StoreCatelogData()
    {
        ...
        try {
            ...
            if (proTitle.length() > 80)
            {
                ...
                VestsProductNamelst.add(proTitle);
        } catch (Exception e) {
            e.printStackTrace();
            VestsProductNamelst.add("N/A");
        }
//        if (waitWithoutExceptionUntilPresentOfElementByLessTime(productPriceBy))
//            VestsProductPricelst.add(productPrice.get(0).getText());
//        else if (waitWithoutExceptionUntilPresentOfElementByLessTime(salePriceBy))
//            VestsProductPricelst.add(salePrice.getText());
//        else
//            VestsProductPricelst.add("N/A");
        try {
          do {
           ...
           while(!waitWithoutExceptionUntilPresentOfElementByLessTime(affiliateShortLinkBy));
            ...
            VestsAffiliateLinks.add(affiliateLink);
        } catch (Exception e) {
            e.printStackTrace();
            VestsAffiliateLinks.add("N/A");
        }
    ...
    private void addingDataToCSV1() throws FileNotFoundException {
        try {
            PrintWriter pw = new PrintWriter(new File("D:\\Haris\\man fashion Web site\\Vests.csv"));
...
//            sb.append(",");
            ...
            sb.append("\r\n");
          for (int i =0; i < (VestsProductNamelst.size()); i++) {
              sb.append(i+1);
              sb.append(",");
              sb.append(VestsProductNamelst.get(i));
//              sb.append(",");
//              sb.append(VestsProductPricelst.get(i));
              sb.append(",");
              sb.append(VestsImageUrllst.get(i));
              sb.append(",");
              sb.append(VestsAffiliateLinks.get(i));
              sb.append("\r\n");
          }
    ...
    private void addingDataToCSV2() throws FileNotFoundException {
        try {
            PrintWriter pw = new PrintWriter(new File("D:\\Haris\\man fashion Web site\\TVests2.csv"));
            StringBuilder sb = new StringBuilder();
            sb.append("Sr.No.");
            sb.append(",");
            sb.append("Product Name");
...
//            sb.append(",");
            ...
            sb.append("Affiliate URL");
            sb.append("\r\n");
            for (int i = (VestsProductNamelst.size()/2); i < (VestsProductNamelst.size()); i++) {
                sb.append(i+1);
                sb.append(",");
                sb.append(VestsProductNamelst.get(i));
//              sb.append(",");
//              sb.append(VestsProductPricelst.get(i));
    ...
    {
        for (int i=0; i < VestsProductNamelst.size(); i++) {
            System.out.println(" "+ VestsProductNamelst.get(i)+"  "+VestsAffiliateLinks.get(i) +"  "+VestsImageUrllst.get(i));
        }
    }
