...
public class WatchesPage extends TestBase {
    ...
    List <String> WatchesProductNamelst = new ArrayList<>();
   // List<String> WatchesProductPricelst = new ArrayList<>();
    List<String> WatchesImageUrllst = new ArrayList<>();
    List<String> WatchesAffiliateLinks = new ArrayList<>();

    String pageLink = "https://www.amazon.co.uk/Best-Sellers-Watches-Mens-Fashion-Smartwatches/zgbs/watch/14284664031/ref=zg_bs_nav_watches_3_10103538031";

  public WatchesPage() throws FileNotFoundException {
        PageFactory.initElements(driver, this);
    }

    private void goToWatchesCatalogPage()
    {
        driver.navigate().to(pageLink);
    }

    public void getWatchesdata() throws FileNotFoundException, InterruptedException {

      goToWatchesCatalogPage();
      waitUntilPageReady();
        ...
        for (int i=1;i<=51;i++) {
            try {
               if(i!=7){
                    ...
                    WatchesImageUrllst.add(imagelnk);
                }
            } catch (Exception e) {
                e.printStackTrace();
                WatchesImageUrllst.add("N/A");
            }
        }
    }
    ...
    private void StoreCatelogData()
    {
        ...
        try {
            ...
            if (proTitle.length() > 80)
            {
                ...
                WatchesProductNamelst.add(proTitle);
        } catch (Exception e) {
            e.printStackTrace();
            WatchesProductNamelst.add("N/A");
        }
//        if (waitWithoutExceptionUntilPresentOfElementByLessTime(productPriceBy))
//            WatchesProductPricelst.add(productPrice.get(0).getText());
//        else if (waitWithoutExceptionUntilPresentOfElementByLessTime(salePriceBy))
//            WatchesProductPricelst.add(salePrice.getText());
//        else
//            WatchesProductPricelst.add("N/A");
        try {
          do {
           ...
           while(!waitWithoutExceptionUntilPresentOfElementByLessTime(affiliateShortLinkBy));
            ...
            WatchesAffiliateLinks.add(affiliateLink);
        } catch (Exception e) {
            e.printStackTrace();
            WatchesAffiliateLinks.add("N/A");
        }
    ...
    private void addingDataToCSV1() throws FileNotFoundException {
        try {
            PrintWriter pw = new PrintWriter(new File("D:\\Haris\\man fashion Web site\\Watches1.csv"));
...
//            sb.append(",");
            ...
            sb.append("\r\n");
          for (int i =0; i < (WatchesProductNamelst.size()/2); i++) {
              sb.append(i+1);
              sb.append(",");
              sb.append(WatchesProductNamelst.get(i));
//              sb.append(",");
//              sb.append(WatchesProductPricelst.get(i));
              sb.append(",");
              sb.append(WatchesImageUrllst.get(i));
              sb.append(",");
              sb.append(WatchesAffiliateLinks.get(i));
              sb.append("\r\n");
          }
    ...
    private void addingDataToCSV2() throws FileNotFoundException {
        try {
            PrintWriter pw = new PrintWriter(new File("D:\\Haris\\man fashion Web site\\Watches2.csv"));
            StringBuilder sb = new StringBuilder();
            sb.append("Sr.No.");
            sb.append(",");
            sb.append("Product Name");
...
//            sb.append(",");
            ...
            sb.append("Affiliate URL");
            sb.append("\r\n");
            for (int i = (WatchesProductNamelst.size()/2); i < (WatchesProductNamelst.size()); i++) {
                sb.append(i+1);
                sb.append(",");
                sb.append(WatchesProductNamelst.get(i));
//              sb.append(",");
//              sb.append(WatchesProductPricelst.get(i));
    ...
    {
        for (int i=0; i < WatchesProductNamelst.size(); i++) {
            System.out.println(" "+ WatchesProductNamelst.get(i)+"  "+WatchesAffiliateLinks.get(i) +"  "+WatchesImageUrllst.get(i));
        }
    }
