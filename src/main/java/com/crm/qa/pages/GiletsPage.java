...
public class GiletsPage extends TestBase {
    ...
    List <String> GiletsProductNamelst = new ArrayList<>();
   // List<String> GiletsProductPricelst = new ArrayList<>();
    List<String> GiletsImageUrllst = new ArrayList<>();
    List<String> GiletsAffiliateLinks = new ArrayList<>();

    String pageLink = "https://www.amazon.co.uk/Best-Sellers-Clothing-Mens-Outerwear-Gilets/zgbs/clothing/1730995031/ref=zg_bs_nav_ap_4_1730994031";

  public GiletsPage() throws FileNotFoundException {
        PageFactory.initElements(driver, this);
    }

    private void goToGiletsCatalogPage()
    {
        driver.navigate().to(pageLink);
    }

    public void getGiletsdata() throws FileNotFoundException, InterruptedException {

      goToGiletsCatalogPage();
      waitUntilPageReady();
        ...
        for (int i=1;i<=26;i++) {
            try {
               if(i!=7){
                    ...
                    GiletsImageUrllst.add(imagelnk);
                }
            } catch (Exception e) {
                e.printStackTrace();
                GiletsImageUrllst.add("N/A");
            }
        }
    }
    ...
    private void StoreCatelogData()
    {
        ...
        try {
            ...
            if (proTitle.length() > 80)
            {
                ...
                GiletsProductNamelst.add(proTitle);
        } catch (Exception e) {
            e.printStackTrace();
            GiletsProductNamelst.add("N/A");
        }
//        if (waitWithoutExceptionUntilPresentOfElementByLessTime(productPriceBy))
//            GiletsProductPricelst.add(productPrice.get(0).getText());
//        else if (waitWithoutExceptionUntilPresentOfElementByLessTime(salePriceBy))
//            GiletsProductPricelst.add(salePrice.getText());
//        else
//            GiletsProductPricelst.add("N/A");
        try {
          do {
           ...
           while(!waitWithoutExceptionUntilPresentOfElementByLessTime(affiliateShortLinkBy));
            ...
            GiletsAffiliateLinks.add(affiliateLink);
        } catch (Exception e) {
            e.printStackTrace();
            GiletsAffiliateLinks.add("N/A");
        }
    ...
    private void addingDataToCSV1() throws FileNotFoundException {
        try {
            PrintWriter pw = new PrintWriter(new File("D:\\Haris\\man fashion Web site\\Gilts.csv"));
...
//            sb.append(",");
            ...
            sb.append("\r\n");
          for (int i =0; i < (GiletsProductNamelst.size()); i++) {
              sb.append(i+1);
              sb.append(",");
              sb.append(GiletsProductNamelst.get(i));
//              sb.append(",");
//              sb.append(GiletsProductPricelst.get(i));
              sb.append(",");
              sb.append(GiletsImageUrllst.get(i));
              sb.append(",");
              sb.append(GiletsAffiliateLinks.get(i));
              sb.append("\r\n");
          }
    ...
    private void addingDataToCSV2() throws FileNotFoundException {
        try {
            PrintWriter pw = new PrintWriter(new File("D:\\Haris\\man fashion Web site\\TGilets2.csv"));
            StringBuilder sb = new StringBuilder();
            sb.append("Sr.No.");
            sb.append(",");
            sb.append("Product Name");
...
//            sb.append(",");
            ...
            sb.append("Affiliate URL");
            sb.append("\r\n");
            for (int i = (GiletsProductNamelst.size()/2); i < (GiletsProductNamelst.size()); i++) {
                sb.append(i+1);
                sb.append(",");
                sb.append(GiletsProductNamelst.get(i));
//              sb.append(",");
//              sb.append(GiletsProductPricelst.get(i));
    ...
    {
        for (int i=0; i < GiletsProductNamelst.size(); i++) {
            System.out.println(" "+ GiletsProductNamelst.get(i)+"  "+GiletsAffiliateLinks.get(i) +"  "+GiletsImageUrllst.get(i));
        }
    }
