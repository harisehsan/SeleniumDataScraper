...
public class BoxerPage extends TestBase {
    ...
    List <String> BoxerProductNamelst = new ArrayList<>();
   // List<String> BoxerProductPricelst = new ArrayList<>();
    List<String> BoxerImageUrllst = new ArrayList<>();
    List<String> BoxerAffiliateLinks = new ArrayList<>();

    String pageLink = "https://www.amazon.co.uk/Best-Sellers-Clothing-Mens-Underwear-Trunks/zgbs/clothing/1731039031/ref=zg_bs_nav_ap_4_1731032031";

  public BoxerPage() throws FileNotFoundException {
        PageFactory.initElements(driver, this);
    }

    private void goToBoxerCatalogPage()
    {
        driver.navigate().to(pageLink);
    }

    public void getBoxerdata() throws FileNotFoundException, InterruptedException {

      goToBoxerCatalogPage();
      waitUntilPageReady();
        ...
        for (int i=1;i<=26;i++) {
            try {
               if(i!=7){
                    ...
                    BoxerImageUrllst.add(imagelnk);
                }
            } catch (Exception e) {
                e.printStackTrace();
                BoxerImageUrllst.add("N/A");
            }
        }
    }
    ...
    private void StoreCatelogData()
    {
        ...
        try {
            ...
            if (proTitle.length() > 80)
            {
                ...
                BoxerProductNamelst.add(proTitle);
        } catch (Exception e) {
            e.printStackTrace();
            BoxerProductNamelst.add("N/A");
        }
//        if (waitWithoutExceptionUntilPresentOfElementByLessTime(productPriceBy))
//            BoxerProductPricelst.add(productPrice.get(0).getText());
//        else if (waitWithoutExceptionUntilPresentOfElementByLessTime(salePriceBy))
//            BoxerProductPricelst.add(salePrice.getText());
//        else
//            BoxerProductPricelst.add("N/A");
        try {
          do {
           ...
           while(!waitWithoutExceptionUntilPresentOfElementByLessTime(affiliateShortLinkBy));
            ...
            BoxerAffiliateLinks.add(affiliateLink);
        } catch (Exception e) {
            e.printStackTrace();
            BoxerAffiliateLinks.add("N/A");
        }
    ...
    private void addingDataToCSV1() throws FileNotFoundException {
        try {
            PrintWriter pw = new PrintWriter(new File("D:\\Haris\\man fashion Web site\\Boxer.csv"));
...
//            sb.append(",");
            ...
            sb.append("\r\n");
          for (int i =0; i < (BoxerProductNamelst.size()); i++) {
              sb.append(i+1);
              sb.append(",");
              sb.append(BoxerProductNamelst.get(i));
//              sb.append(",");
//              sb.append(BoxerProductPricelst.get(i));
              sb.append(",");
              sb.append(BoxerImageUrllst.get(i));
              sb.append(",");
              sb.append(BoxerAffiliateLinks.get(i));
              sb.append("\r\n");
          }
    ...
    private void addingDataToCSV2() throws FileNotFoundException {
        try {
            PrintWriter pw = new PrintWriter(new File("D:\\Haris\\man fashion Web site\\TBoxer2.csv"));
            StringBuilder sb = new StringBuilder();
            sb.append("Sr.No.");
            sb.append(",");
            sb.append("Product Name");
...
//            sb.append(",");
            ...
            sb.append("Affiliate URL");
            sb.append("\r\n");
            for (int i = (BoxerProductNamelst.size()/2); i < (BoxerProductNamelst.size()); i++) {
                sb.append(i+1);
                sb.append(",");
                sb.append(BoxerProductNamelst.get(i));
//              sb.append(",");
//              sb.append(BoxerProductPricelst.get(i));
    ...
    {
        for (int i=0; i < BoxerProductNamelst.size(); i++) {
            System.out.println(" "+ BoxerProductNamelst.get(i)+"  "+BoxerAffiliateLinks.get(i) +"  "+BoxerImageUrllst.get(i));
        }
    }
