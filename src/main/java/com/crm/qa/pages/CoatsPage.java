...
public class CoatsPage extends TestBase {
    ...
    List <String> CoatsProductNamelst = new ArrayList<>();
   // List<String> CoatsProductPricelst = new ArrayList<>();
    List<String> CoatsImageUrllst = new ArrayList<>();
    List<String> CoatsAffiliateLinks = new ArrayList<>();

    String pageLink = "https://www.amazon.co.uk/Best-Sellers-Clothing-Mens-Coats/zgbs/clothing/1730994031/ref=zg_bs_nav_ap_3_1730993031";

  public CoatsPage() throws FileNotFoundException {
        PageFactory.initElements(driver, this);
    }

    private void goToCoatsCatalogPage()
    {
        driver.navigate().to(pageLink);
    }

    public void getCoatsdata() throws FileNotFoundException, InterruptedException {

      goToCoatsCatalogPage();
      waitUntilPageReady();
        ...
        for (int i=1;i<=26;i++) {
            try {
               if(i!=7){
                    ...
                    CoatsImageUrllst.add(imagelnk);
                }
            } catch (Exception e) {
                e.printStackTrace();
                CoatsImageUrllst.add("N/A");
            }
        }
    }
    ...
    private void StoreCatelogData()
    {
        ...
        try {
            ...
            if (proTitle.length() > 80)
            {
                ...
                CoatsProductNamelst.add(proTitle);
        } catch (Exception e) {
            e.printStackTrace();
            CoatsProductNamelst.add("N/A");
        }
//        if (waitWithoutExceptionUntilPresentOfElementByLessTime(productPriceBy))
//            CoatsProductPricelst.add(productPrice.get(0).getText());
//        else if (waitWithoutExceptionUntilPresentOfElementByLessTime(salePriceBy))
//            CoatsProductPricelst.add(salePrice.getText());
//        else
//            CoatsProductPricelst.add("N/A");
        try {
          do {
           ...
           while(!waitWithoutExceptionUntilPresentOfElementByLessTime(affiliateShortLinkBy));
            ...
            CoatsAffiliateLinks.add(affiliateLink);
        } catch (Exception e) {
            e.printStackTrace();
            CoatsAffiliateLinks.add("N/A");
        }
    ...
    private void addingDataToCSV1() throws FileNotFoundException {
        try {
            PrintWriter pw = new PrintWriter(new File("D:\\Haris\\man fashion Web site\\Coats.csv"));
...
//            sb.append(",");
            ...
            sb.append("\r\n");
          for (int i =0; i < (CoatsProductNamelst.size()); i++) {
              sb.append(i+1);
              sb.append(",");
              sb.append(CoatsProductNamelst.get(i));
//              sb.append(",");
//              sb.append(CoatsProductPricelst.get(i));
              sb.append(",");
              sb.append(CoatsImageUrllst.get(i));
              sb.append(",");
              sb.append(CoatsAffiliateLinks.get(i));
              sb.append("\r\n");
          }
    ...
    private void addingDataToCSV2() throws FileNotFoundException {
        try {
            PrintWriter pw = new PrintWriter(new File("D:\\Haris\\man fashion Web site\\TCoats2.csv"));
            StringBuilder sb = new StringBuilder();
            sb.append("Sr.No.");
            sb.append(",");
            sb.append("Product Name");
...
//            sb.append(",");
            ...
            sb.append("Affiliate URL");
            sb.append("\r\n");
            for (int i = (CoatsProductNamelst.size()/2); i < (CoatsProductNamelst.size()); i++) {
                sb.append(i+1);
                sb.append(",");
                sb.append(CoatsProductNamelst.get(i));
//              sb.append(",");
//              sb.append(CoatsProductPricelst.get(i));
    ...
    {
        for (int i=0; i < CoatsProductNamelst.size(); i++) {
            System.out.println(" "+ CoatsProductNamelst.get(i)+"  "+CoatsAffiliateLinks.get(i) +"  "+CoatsImageUrllst.get(i));
        }
    }
