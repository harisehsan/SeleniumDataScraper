...
public class JeansPage extends TestBase {
    ...
    List <String> JeansProductNamelst = new ArrayList<>();
   // List<String> JeansProductPricelst = new ArrayList<>();
    List<String> JeansImageUrllst = new ArrayList<>();
    List<String> JeansAffiliateLinks = new ArrayList<>();
    
    String pageLink = "https://www.amazon.co.uk/Best-Sellers-Clothing-Mens-Jeans/zgbs/clothing/1730981031/ref=zg_bs_nav_ap_2_1730929031";

  public JeansPage() throws FileNotFoundException {
        PageFactory.initElements(driver, this);
    }

    private void goToJeansCatalogPage()
    {
        driver.navigate().to(pageLink);
    }

    public void getJeansdata() throws FileNotFoundException, InterruptedException {

      goToJeansCatalogPage();
      waitUntilPageReady();
        ...
        for (int i=1;i<=51;i++) {
            try {
               if(i!=7){
                    ...
                    JeansImageUrllst.add(imagelnk);
                }
            } catch (Exception e) {
                e.printStackTrace();
                JeansImageUrllst.add("N/A");
            }
        }
    }
    ...
    private void StoreCatelogData()
    {
        ...
        try {
            ...
            if (proTitle.length() > 80)
            {
                ...
                JeansProductNamelst.add(proTitle);
        } catch (Exception e) {
            e.printStackTrace();
            JeansProductNamelst.add("N/A");
        }
//        if (waitWithoutExceptionUntilPresentOfElementByLessTime(productPriceBy))
//            JeansProductPricelst.add(productPrice.get(0).getText());
//        else if (waitWithoutExceptionUntilPresentOfElementByLessTime(salePriceBy))
//            JeansProductPricelst.add(salePrice.getText());
//        else
//            JeansProductPricelst.add("N/A");
        try {
          do {
           ...
           while(!waitWithoutExceptionUntilPresentOfElementByLessTime(affiliateShortLinkBy));
            ...
            JeansAffiliateLinks.add(affiliateLink);
        } catch (Exception e) {
            e.printStackTrace();
            JeansAffiliateLinks.add("N/A");
        }
    ...
    private void addingDataToCSV1() throws FileNotFoundException {
        try {
            PrintWriter pw = new PrintWriter(new File("D:\\Haris\\man fashion Web site\\Jeans1.csv"));
...
//            sb.append(",");
            ...
            sb.append("\r\n");
          for (int i =0; i < (JeansProductNamelst.size()/2); i++) {
              sb.append(i+1);
              sb.append(",");
              sb.append(JeansProductNamelst.get(i));
//              sb.append(",");
//              sb.append(JeansProductPricelst.get(i));
              sb.append(",");
              sb.append(JeansImageUrllst.get(i));
              sb.append(",");
              sb.append(JeansAffiliateLinks.get(i));
              sb.append("\r\n");
          }
    ...
    private void addingDataToCSV2() throws FileNotFoundException {
        try {
            PrintWriter pw = new PrintWriter(new File("D:\\Haris\\man fashion Web site\\Jeans2.csv"));
            StringBuilder sb = new StringBuilder();
            sb.append("Sr.No.");
            sb.append(",");
            sb.append("Product Name");
...
//            sb.append(",");
            ...
            sb.append("Affiliate URL");
            sb.append("\r\n");
            for (int i = (JeansProductNamelst.size()/2); i < (JeansProductNamelst.size()); i++) {
                sb.append(i+1);
                sb.append(",");
                sb.append(JeansProductNamelst.get(i));
//              sb.append(",");
//              sb.append(JeansProductPricelst.get(i));
    ...
    {
        for (int i=0; i < JeansProductNamelst.size(); i++) {
            System.out.println(" "+ JeansProductNamelst.get(i)+"  "+JeansAffiliateLinks.get(i) +"  "+JeansImageUrllst.get(i));
        }
    }
