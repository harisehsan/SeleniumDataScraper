...
public class SweatshirtsPage extends TestBase {
    ...
    List <String> SweatshirtsProductNamelst = new ArrayList<>();
   // List<String> SweatshirtsProductPricelst = new ArrayList<>();
    List<String> SweatshirtsImageUrllst = new ArrayList<>();
    List<String> SweatshirtsAffiliateLinks = new ArrayList<>();

    String pageLink = "https://www.amazon.co.uk/Best-Sellers-Clothing-Mens-Sweatshirts/zgbs/clothing/1731021031/ref=zg_bs_nav_ap_4_14704068031";

  public SweatshirtsPage() throws FileNotFoundException {
        PageFactory.initElements(driver, this);
    }

    private void goToSweatshirtsCatalogPage()
    {
        driver.navigate().to(pageLink);
    }

    public void getSweatshirtsdata() throws FileNotFoundException, InterruptedException {

      goToSweatshirtsCatalogPage();
      waitUntilPageReady();
        ...
        for (int i=1;i<=26;i++) {
            try {
               if(i!=7){
                    ...
                    SweatshirtsImageUrllst.add(imagelnk);
                }
            } catch (Exception e) {
                e.printStackTrace();
                SweatshirtsImageUrllst.add("N/A");
            }
        }
    }
    ...
    private void StoreCatelogData()
    {
        ...
        try {
            ...
            if (proTitle.length() > 80)
            {
                ...
                SweatshirtsProductNamelst.add(proTitle);
        } catch (Exception e) {
            e.printStackTrace();
            SweatshirtsProductNamelst.add("N/A");
        }
//        if (waitWithoutExceptionUntilPresentOfElementByLessTime(productPriceBy))
//            SweatshirtsProductPricelst.add(productPrice.get(0).getText());
//        else if (waitWithoutExceptionUntilPresentOfElementByLessTime(salePriceBy))
//            SweatshirtsProductPricelst.add(salePrice.getText());
//        else
//            SweatshirtsProductPricelst.add("N/A");
        try {
          do {
           ...
           while(!waitWithoutExceptionUntilPresentOfElementByLessTime(affiliateShortLinkBy));
            ...
            SweatshirtsAffiliateLinks.add(affiliateLink);
        } catch (Exception e) {
            e.printStackTrace();
            SweatshirtsAffiliateLinks.add("N/A");
        }
    ...
    private void addingDataToCSV1() throws FileNotFoundException {
        try {
            PrintWriter pw = new PrintWriter(new File("D:\\Haris\\man fashion Web site\\Sweatshirts.csv"));
...
//            sb.append(",");
            ...
            sb.append("\r\n");
          for (int i =0; i < (SweatshirtsProductNamelst.size()); i++) {
              sb.append(i+1);
              sb.append(",");
              sb.append(SweatshirtsProductNamelst.get(i));
//              sb.append(",");
//              sb.append(SweatshirtsProductPricelst.get(i));
              sb.append(",");
              sb.append(SweatshirtsImageUrllst.get(i));
              sb.append(",");
              sb.append(SweatshirtsAffiliateLinks.get(i));
              sb.append("\r\n");
          }
    ...
    private void addingDataToCSV2() throws FileNotFoundException {
        try {
            PrintWriter pw = new PrintWriter(new File("D:\\Haris\\man fashion Web site\\TSweatshirts2.csv"));
            StringBuilder sb = new StringBuilder();
            sb.append("Sr.No.");
            sb.append(",");
            sb.append("Product Name");
...
//            sb.append(",");
            ...
            sb.append("Affiliate URL");
            sb.append("\r\n");
            for (int i = (SweatshirtsProductNamelst.size()/2); i < (SweatshirtsProductNamelst.size()); i++) {
                sb.append(i+1);
                sb.append(",");
                sb.append(SweatshirtsProductNamelst.get(i));
//              sb.append(",");
//              sb.append(SweatshirtsProductPricelst.get(i));
    ...
    {
        for (int i=0; i < SweatshirtsProductNamelst.size(); i++) {
            System.out.println(" "+ SweatshirtsProductNamelst.get(i)+"  "+SweatshirtsAffiliateLinks.get(i) +"  "+SweatshirtsImageUrllst.get(i));
        }
    }
