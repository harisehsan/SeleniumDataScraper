...
public class JacketsPage extends TestBase {
    ...
    List <String> JacketsProductNamelst = new ArrayList<>();
   // List<String> JacketsProductPricelst = new ArrayList<>();
    List<String> JacketsImageUrllst = new ArrayList<>();
    List<String> JacketsAffiliateLinks = new ArrayList<>();

    String pageLink = "https://www.amazon.co.uk/Best-Sellers-Clothing-Mens-Jackets/zgbs/clothing/1730996031/ref=zg_bs_nav_ap_3_1730993031";

  public JacketsPage() throws FileNotFoundException {
        PageFactory.initElements(driver, this);
    }

    private void goToJacketsCatalogPage()
    {
        driver.navigate().to(pageLink);
    }

    public void getJacketsdata() throws FileNotFoundException, InterruptedException {

      goToJacketsCatalogPage();
      waitUntilPageReady();
        ...
        for (int i=1;i<=51;i++) {
            try {
               if(i!=7){
                    ...
                    JacketsImageUrllst.add(imagelnk);
                }
            } catch (Exception e) {
                e.printStackTrace();
                JacketsImageUrllst.add("N/A");
            }
        }
    }
    ...
    private void StoreCatelogData()
    {
        ...
        try {
            ...
            if (proTitle.length() > 80)
            {
                ...
                JacketsProductNamelst.add(proTitle);
        } catch (Exception e) {
            e.printStackTrace();
            JacketsProductNamelst.add("N/A");
        }
//        if (waitWithoutExceptionUntilPresentOfElementByLessTime(productPriceBy))
//            JacketsProductPricelst.add(productPrice.get(0).getText());
//        else if (waitWithoutExceptionUntilPresentOfElementByLessTime(salePriceBy))
//            JacketsProductPricelst.add(salePrice.getText());
//        else
//            JacketsProductPricelst.add("N/A");
        try {
          do {
           ...
           while(!waitWithoutExceptionUntilPresentOfElementByLessTime(affiliateShortLinkBy));
            ...
            JacketsAffiliateLinks.add(affiliateLink);
        } catch (Exception e) {
            e.printStackTrace();
            JacketsAffiliateLinks.add("N/A");
        }
    ...
    private void addingDataToCSV1() throws FileNotFoundException {
        try {
            PrintWriter pw = new PrintWriter(new File("D:\\Haris\\man fashion Web site\\Jackets1.csv"));
...
//            sb.append(",");
            ...
            sb.append("\r\n");
          for (int i =0; i < (JacketsProductNamelst.size()/2); i++) {
              sb.append(i+1);
              sb.append(",");
              sb.append(JacketsProductNamelst.get(i));
//              sb.append(",");
//              sb.append(JacketsProductPricelst.get(i));
              sb.append(",");
              sb.append(JacketsImageUrllst.get(i));
              sb.append(",");
              sb.append(JacketsAffiliateLinks.get(i));
              sb.append("\r\n");
          }
    ...
    private void addingDataToCSV2() throws FileNotFoundException {
        try {
            PrintWriter pw = new PrintWriter(new File("D:\\Haris\\man fashion Web site\\Jackets2.csv"));
            StringBuilder sb = new StringBuilder();
            sb.append("Sr.No.");
            sb.append(",");
            sb.append("Product Name");
...
//            sb.append(",");
            ...
            sb.append("Affiliate URL");
            sb.append("\r\n");
            for (int i = (JacketsProductNamelst.size()/2); i < (JacketsProductNamelst.size()); i++) {
                sb.append(i+1);
                sb.append(",");
                sb.append(JacketsProductNamelst.get(i));
//              sb.append(",");
//              sb.append(JacketsProductPricelst.get(i));
    ...
    {
        for (int i=0; i < JacketsProductNamelst.size(); i++) {
            System.out.println(" "+ JacketsProductNamelst.get(i)+"  "+JacketsAffiliateLinks.get(i) +"  "+JacketsImageUrllst.get(i));
        }
    }
