...
public class HatsPage extends TestBase {
    ...
    List <String> HatsProductNamelst = new ArrayList<>();
   // List<String> HatsProductPricelst = new ArrayList<>();
    List<String> HatsImageUrllst = new ArrayList<>();
    List<String> HatsAffiliateLinks = new ArrayList<>();

    String pageLink = "https://www.amazon.co.uk/Best-Sellers-Clothing-Mens-Hats-Caps/zgbs/clothing/1730944031/ref=zg_bs_nav_ap_3_1730930031";

  public HatsPage() throws FileNotFoundException {
        PageFactory.initElements(driver, this);
    }

    private void goToHatsCatalogPage()
    {
        driver.navigate().to(pageLink);
    }

    public void getHatsdata() throws FileNotFoundException, InterruptedException {
        ...
        for (int i=1;i<=26;i++) {
            try {
               if(i!=7){
                    ...
                    HatsImageUrllst.add(imagelnk);
                }
            } catch (Exception e) {
                e.printStackTrace();
                HatsImageUrllst.add("N/A");
            }
        }
    }
    ...
    private void StoreCatelogData()
    {
        ...
        try {
            ...
            if (proTitle.length() > 80)
            {
                ...
                HatsProductNamelst.add(proTitle);
        } catch (Exception e) {
            e.printStackTrace();
            HatsProductNamelst.add("N/A");
        }
//        if (waitWithoutExceptionUntilPresentOfElementByLessTime(productPriceBy))
//            HatsProductPricelst.add(productPrice.get(0).getText());
//        else if (waitWithoutExceptionUntilPresentOfElementByLessTime(salePriceBy))
//            HatsProductPricelst.add(salePrice.getText());
//        else
//            HatsProductPricelst.add("N/A");
        try {
          do {
           ...
           while(!waitWithoutExceptionUntilPresentOfElementByLessTime(affiliateShortLinkBy));
            ...
            HatsAffiliateLinks.add(affiliateLink);
        } catch (Exception e) {
            e.printStackTrace();
            HatsAffiliateLinks.add("N/A");
        }
    ...
    private void addingDataToCSV1() throws FileNotFoundException {
        try {
            PrintWriter pw = new PrintWriter(new File("D:\\Haris\\man fashion Web site\\Hats.csv"));
...
//            sb.append(",");
            ...
            sb.append("\r\n");
          for (int i =0; i < (HatsProductNamelst.size()); i++) {
              sb.append(i+1);
              sb.append(",");
              sb.append(HatsProductNamelst.get(i));
//              sb.append(",");
//              sb.append(HatsProductPricelst.get(i));
              sb.append(",");
              sb.append(HatsImageUrllst.get(i));
              sb.append(",");
              sb.append(HatsAffiliateLinks.get(i));
              sb.append("\r\n");
          }
    ...
    private void addingDataToCSV2() throws FileNotFoundException {
        try {
            PrintWriter pw = new PrintWriter(new File("D:\\Haris\\man fashion Web site\\THats2.csv"));
            StringBuilder sb = new StringBuilder();
            sb.append("Sr.No.");
            sb.append(",");
            sb.append("Product Name");
...
//            sb.append(",");
            ...
            sb.append("Affiliate URL");
            sb.append("\r\n");
            for (int i = (HatsProductNamelst.size()/2); i < (HatsProductNamelst.size()); i++) {
                sb.append(i+1);
                sb.append(",");
                sb.append(HatsProductNamelst.get(i));
//              sb.append(",");
//              sb.append(HatsProductPricelst.get(i));
    ...
    {
        for (int i=0; i < HatsProductNamelst.size(); i++) {
            System.out.println(" "+ HatsProductNamelst.get(i)+"  "+HatsAffiliateLinks.get(i) +"  "+HatsImageUrllst.get(i));
        }
    }
