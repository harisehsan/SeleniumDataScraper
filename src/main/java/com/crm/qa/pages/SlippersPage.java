...
public class SlippersPage extends TestBase {
    ...
    List <String> SlippersProductNamelst = new ArrayList<>();
   // List<String> SlippersProductPricelst = new ArrayList<>();
    List<String> SlippersImageUrllst = new ArrayList<>();
    List<String> SlippersAffiliateLinks = new ArrayList<>();

    String pageLink = "https://www.amazon.co.uk/gp/bestsellers/shoes/1769795031/ref=pd_zg_hrsr_shoes";

  public SlippersPage() throws FileNotFoundException {
        PageFactory.initElements(driver, this);
    }

    private void goToSlippersCatalogPage()
    {
        driver.navigate().to(pageLink);
    }

    public void getSlippersdata() throws FileNotFoundException, InterruptedException {

      goToSlippersCatalogPage();
      waitUntilPageReady();
        ...
        for (int i=1;i<=51;i++) {
            try {
               if(i!=7){
                    ...
                    SlippersImageUrllst.add(imagelnk);
                }
            } catch (Exception e) {
                e.printStackTrace();
                SlippersImageUrllst.add("N/A");
            }
        }
    }
    ...
    private void StoreCatelogData()
    {
        ...
        try {
            ...
            if (proTitle.length() > 80)
            {
                ...
                SlippersProductNamelst.add(proTitle);
        } catch (Exception e) {
            e.printStackTrace();
            SlippersProductNamelst.add("N/A");
        }
//        if (waitWithoutExceptionUntilPresentOfElementByLessTime(productPriceBy))
//            SlippersProductPricelst.add(productPrice.get(0).getText());
//        else if (waitWithoutExceptionUntilPresentOfElementByLessTime(salePriceBy))
//            SlippersProductPricelst.add(salePrice.getText());
//        else
//            SlippersProductPricelst.add("N/A");
        try {
          do {
           ...
           while(!waitWithoutExceptionUntilPresentOfElementByLessTime(affiliateShortLinkBy));
            ...
            SlippersAffiliateLinks.add(affiliateLink);
        } catch (Exception e) {
            e.printStackTrace();
            SlippersAffiliateLinks.add("N/A");
        }
    ...
    private void addingDataToCSV1() throws FileNotFoundException {
        try {
            PrintWriter pw = new PrintWriter(new File("D:\\Haris\\man fashion Web site\\Slippers1.csv"));
...
//            sb.append(",");
            ...
            sb.append("\r\n");
          for (int i =0; i < (SlippersProductNamelst.size()/2); i++) {
              sb.append(i+1);
              sb.append(",");
              sb.append(SlippersProductNamelst.get(i));
//              sb.append(",");
//              sb.append(SlippersProductPricelst.get(i));
              sb.append(",");
              sb.append(SlippersImageUrllst.get(i));
              sb.append(",");
              sb.append(SlippersAffiliateLinks.get(i));
              sb.append("\r\n");
          }
    ...
    private void addingDataToCSV2() throws FileNotFoundException {
        try {
            PrintWriter pw = new PrintWriter(new File("D:\\Haris\\man fashion Web site\\Slippers2.csv"));
            StringBuilder sb = new StringBuilder();
            sb.append("Sr.No.");
            sb.append(",");
            sb.append("Product Name");
...
//            sb.append(",");
            ...
            sb.append("Affiliate URL");
            sb.append("\r\n");
            for (int i = (SlippersProductNamelst.size()/2); i < (SlippersProductNamelst.size()); i++) {
                sb.append(i+1);
                sb.append(",");
                sb.append(SlippersProductNamelst.get(i));
//              sb.append(",");
//              sb.append(SlippersProductPricelst.get(i));
    ...
    {
        for (int i=0; i < SlippersProductNamelst.size(); i++) {
            System.out.println(" "+ SlippersProductNamelst.get(i)+"  "+SlippersAffiliateLinks.get(i) +"  "+SlippersImageUrllst.get(i));
        }
    }
